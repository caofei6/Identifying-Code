package com.IdentifyingCode.caofei;
...
public class DrawImage extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		BufferedImage bi = new BufferedImage(WIDTH, HEIGHT,
				BufferedImage.TYPE_INT_BGR); // 在内存中创建一张图片
		Graphics g = bi.getGraphics();        //得到图片

		setBackGroud(g);         //设置背景色
		setBorder(g);            //设置图片的边框
		drawRandomLine(g);      //在图片上画干扰线

		String random = drawRandomNum((Graphics2D) g, createTypeFlag);   //根据客户端传递的creatTypeFlag标识生成验证码图片

		request.getSession().setAttribute("checkcode", random);
		response.setContentType("image/jpeg");
		// 设置响应头控制浏览器不要缓存
		response.setDateHeader("expreoes", -1);
	 ...
	 * @param g
	 */
	public void setBackGroud(Graphics g) {
